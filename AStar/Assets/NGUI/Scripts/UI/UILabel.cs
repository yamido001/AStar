...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] NumShowType mNumShowType = NumShowType.None;
	...
	public NumShowType numberShowType
	{
		get
		{
			return mNumShowType;
	...
	protected virtual void ProcessText (bool legacyMode, bool full)
	{
		if (!isValid) return;
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				mProcessedText = FormateNumberText(mProcessedText, mNumShowType);

				if (mOverflow == Overflow.EllipsisContent && !fits && mText.Length != mProcessedText.Length)
				{
					var truncated = mProcessedText;
					bool firstTry = true;
					int len = mProcessedText.Length;
					var sb = new System.Text.StringBuilder(truncated);
					for (int i = 0; i < len; ++i)
					{
						sb.Remove(sb.Length - (firstTry ? 1 : 4), (firstTry ? 1 : 4));
						firstTry = false;
						sb.Append(ellipsis);
						var candidate = sb.ToString();
						if (NGUIText.WrapText(candidate, out mProcessedText, true)
						    || candidate.Length == mProcessedText.Length)
						{
							return;
						}
					}
	...
	public static string FormateNumberText(string input, NumShowType showType)
	{
		long number = 0;
		...
		if(showType == NumShowType.Full)
		{
			return string.Format("{0:N0}", number);
		}
		else if(showType == NumShowType.Short)
		{
			if(Math.Abs(number) >= 1000000000)
			{
				long temp = number / 100000000;
				float newNum = temp / 10f;
				return string.Format("{0:F1}B", newNum);
			}
			else if(Math.Abs(number) >= 1000000)
			{
				long temp = number / 100000;
				float newNum = temp / 10f;
				return string.Format("{0:F1}M", newNum);
			}
			else if(Math.Abs(number) >= 1000)
			{
				long temp = number / 100;
				float newNum = temp / 10f;
				return string.Format("{0:F1}K", newNum);
			}
			else
			{
				return input;
		...
		else if (showType == NumShowType.K_Short)
		{
