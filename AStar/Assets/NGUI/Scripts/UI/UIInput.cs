...
{
	public const int UnicodeEnglishEnd = 12287;
	...
	// add selected event
	public List<EventDelegate> onSelected = new List<EventDelegate> ();

	// add deselected event
	public List<EventDelegate> onDeSelected = new List<EventDelegate> ();
	...
	[System.NonSerialized] protected int mCurLength = 0;
	...
	public string Validate (string val)
	{
		...
		for (int i = 0; i < val.Length; ++i)
		{
			...
			if (c == 0) continue;
			curCharLength = CalCharLength(c);
			curLength += curCharLength;
			if (characterLimit > 0 && characterLimit < curLength)
			{
				curLength -= curCharLength;
				break;
			}
	...
	protected void OnSelectEvent ()
	{
		...
		EventDelegate.Execute(onSelected);
	}
	...
	protected virtual void Insert (string text)
	{
		...
		int rl = CalculateStringLength(right);

		StringBuilder sb = new StringBuilder(left.Length + right.Length + text.Length);
		sb.Append(left);

		// Append the new text
		int cl = CalculateStringLength (left);
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			// If we have an input validator, validate the input first
			char c = text[i];
			int tl = CalCharLength(c);
			...
			if (characterLimit > 0 && cl + tl + rl > characterLimit) break;
	...
	/// </summary>
	protected int CalculateStringLength(string text)
	{
		int totalLength = 0;
		for (int i = 0; i < text.Length; ++i) {
			char ch = text[i];
			totalLength += CalCharLength(ch);
		}
		return totalLength;
	}
	...
	/// </summary>
	protected static int CalCharLength(char ch)
	{
		return ch > UnicodeEnglishEnd ? 2 : 1;
	}

	public static string InterceptionByLength(string text, int lengthLimit)
	{
		StringBuilder strBuilder = new StringBuilder (Mathf.Min (text.Length, lengthLimit));
		int curLength = 0;
		for (int i = 0; i < text.Length; ++i) {
			char ch = text[i];
			curLength += CalCharLength(ch);
			if (curLength > lengthLimit)
				break;
