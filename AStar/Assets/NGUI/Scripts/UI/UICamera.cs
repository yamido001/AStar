...
public class UICamera : MonoBehaviour
{
	...
	public static float clickIntervalTime = 0f;
	static float mLastClick = 0f;
	/// <summary>
	/// Generic notification function. Used in place of SendMessage to shorten the code and allow for more than one receiver.
	/// </summary>
	public static System.Action<GameObject, string, object> hdlNGUIInput;

	public delegate bool EvtResponseChecker(GameObject go);
	public static EvtResponseChecker hdlGenericResponseChecker;
	public static bool IsResponsive(GameObject go){
		return hdlGenericResponseChecker == null || hdlGenericResponseChecker.Invoke(go);
	}
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		if(funcName == "OnClick" && clickIntervalTime > 0f)
		{
			float time = RealTime.time;
			if (mLastClick + clickIntervalTime > RealTime.time)
			{
				mNotifying = false;
				return;
			}
		...
		if (NGUITools.GetActive(go))
		{
			...
			if (hdlNGUIInput != null){
				hdlNGUIInput.Invoke(go, funcName, obj);
			}
		}
		mNotifying = false;
	}
	...
	/// 通知3dtouch按压信息
	/// </summary>
	public void Notify3DTouchPress()
	{
		if( mCurrentSelection != null )
		{
			this.Notify(mCurrentSelection, "On3DTouchPress", null);
		}
	}
